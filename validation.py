...
#
# The GUI is not part of this autmated test, due to the difficulty 
...
if __name__ == '__main__':
	...
	validateThisNightmare = [
		Test(
			...
			command = 'python "$DUT" --no-gui --bench validation.py --dut "$DUT" --suite successTests',
		...
		Test(
			...
			command = 'python "$DUT" --no-gui --bench validation.py --dut "$DUT" --suite failTests -c',
		...
		Test(
			...
			command = 'python "$DUT" --no-gui --bench validation.py --suite successTests',
			stdout = 'lambda r: r.find("I ran 6 out of 6 tests in total") > 0 and r.find("Success: 6") > 0',
			returnCode = 0
		),
		Test(
			...
			command = 'python "$DUT" --no-gui --bench validation.py --suite failTests -c',
			stdout = 'lambda r: r.find("I ran 6 out of 6 tests in total") > 0 and r.find("Failed: 6") > 0',
			returnCode = 1
		),
		Test(
	...
	successTests = [
		...
		Test(
			...
			description = 'Test if the comparison with regulary expressions works correctly',
			command = 'echo this should be a success',
